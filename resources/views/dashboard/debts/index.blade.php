            ...
            <tbody>
              @forelse ($debts as $debt)
                <tr class="border-b dark:border-gray-700">
                  ...
                  <td class="px-4 py-3">
                    @if ($debt->status === 'paid')
                      <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">Pagada</span>
                    @else
                  ...
                  <td class="px-4 py-3 flex items-center justify-end overflow-visible">
                    ...
                    <div id="debt-{{ $debt->id }}-dropdown"
                      ...
                      <ul class="py-1 text-sm" aria-labelledby="debt-{{ $debt->id }}-dropdown-button">
                        <li>
                          @if ($debt->status !== 'paid')
                            <button type="button" data-modal-target="updateDebtModal"
                              ...
                              data-end-date="{{ $debt->end_date->format('Y-m-d') }}"
                        ...
                        <li>
                          <button type="button" data-modal-target="readDebtModal"
                            data-modal-toggle="readDebtModal" data-id="{{ $debt->id }}"
                            data-lender="{{ $debt->lender }}" data-total-amount="{{ $debt->total_amount }}"
                            data-paid-amount="{{ $debt->paid_amount }}"
                            data-start-date="{{ $debt->start_date->format('d/m/Y') }}"
                            data-end-date="{{ $debt->end_date->format('d/m/Y') }}"
                            data-status="{{ $debt->status }}" data-progress="{{ $debt->progress }}"
                            class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                            <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20"
                              fill="currentColor" aria-hidden="true">
                        ...
                        <li>
                          @if ($debt->status !== 'paid')
                            <button type="button" data-modal-target="paymentDebtModal"
                              ...
                              <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20"
                                fill="currentColor" aria-hidden="true">
                                <path d="M4 4a1 1 0 00-1 1v6a1 1 0 001 1h3.586l-1.293 1.293a1 1 0 001.414 1.414l2-2a1 1 0 000-1.414l-2-2a1 1 0 00-1.414 0L5.414 10H4zm9 1a1 1 0 011 1v3a1 1 0 11-2 0v-3a1 1 0 011-1zM5 13a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1z"
                                  clip-rule="evenodd" />
                              </svg>
                              Agregar Pago
                            </button>
...
Eliminar
              ...
              @empty
                <tr>
                  <td colspan="8" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No se encontraron deudas.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
        ...
        <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
          ...
          <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
            Mostrando
            <span class="font-semibold text-gray-900 dark:text-white">{{ $debts->firstItem() ?? 0 }}-{{ $debts->lastItem() ?? 0 }}</span>
            de
        ...
        <form id="createDebtForm" action="{{ route('dashboard.debts.store') }}" method="POST">
          ...
          <div class="grid gap-4 mb-4 grid-cols-2">
            <div class="col-span-2">
              <label for="lender" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Prestamista</label>
            ...
            <div>
              <label for="total_amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Monto Total</label>
              <input type="number" name="total_amount" id="total_amount" step="0.01" min="0" value="{{ old('total_amount') }}"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
            ...
            <div>
              <label for="paid_amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Monto Pagado (Opcional)</label>
            ...
            <div>
              <label for="end_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fecha de Fin</label>
    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full">
      <div class="relative p-4 bg-white rounded-lg shadow border border-gray-200 dark:border-gray-600 dark:bg-gray-800 sm:p-5">
        ...
        <form id="updateDebtForm" action="" method="POST">
          ...
          <div class="grid gap-4 mb-4 grid-cols-2">
            <div class="col-span-2">
              <label for="update-lender" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Prestamista</label>
              <input type="text" name="lender" id="update-lender"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                required>
            </div>
            <div>
              <label for="update-total_amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Monto Total</label>
              <input type="number" name="total_amount" id="update-total_amount" step="0.01" min="0"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                required>
            </div>
            <div>
              <label for="update-paid_amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Monto Pagado</label>
              <input type="number" name="paid_amount" id="update-paid_amount" step="0.01" min="0"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div>
              <label for="update-start_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fecha de Inicio</label>
              <input type="date" name="start_date" id="update-start_date"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                required>
            </div>
            <div>
              <label for="update-end_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fecha de Fin</label>
      ...
      <div class="relative p-4 bg-white rounded-lg shadow border border-gray-200 dark:border-gray-600 dark:bg-gray-800 sm:p-5">
        <div class="flex justify-between mb-4 rounded-t">
          <div class="text-lg text-gray-900 md:text-xl dark:text-white">
            <h3 class="font-semibold" id="read-lender"></h3>
          </div>
          <div>
            <button type="button"
              class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex dark:hover:bg-gray-600 dark:hover:text-white"
              data-modal-toggle="readDebtModal">
              <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewbox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd"
                  d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
        ...
        <div class="flex justify-between items-center">
          <button type="button"
            ...
            data-modal-toggle="readDebtModal">Cerrar</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative p-4 bg-white rounded-lg shadow border border-gray-200 dark:border-gray-600 dark:bg-gray-800 sm:p-5">
        ...
        <form id="paymentDebtForm" action="" method="POST">
          @csrf
          <div class="mb-4">
            <label for="payment_amount" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Monto del Pago</label>
  ...
  <div id="deleteDebtModal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full bg-white rounded-lg shadow border border-gray-200 dark:border-gray-600 dark:bg-gray-800 sm:p-5 my-4">
      ...
      <p class="mb-4 text-gray-500 dark:text-gray-300">¿Estás seguro de que deseas eliminar "<span id="delete-debt-name"></span>"?</p>
      <div class="flex justify-center items-center space-x-4">
        ...
        <form id="deleteDebtForm" method="POST" class="inline">
          ...
          @method('DELETE')
          <button type="submit"
            class="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">Sí, eliminar</button>
        </form>
      </div>
    </div>
  </div>
    ...
    <script>
      ...
      document.querySelectorAll('[data-modal-target="readDebtModal"][data-modal-toggle]').forEach(button => {
        button.addEventListener('click', function() {
          ...
          document.getElementById('read-total-amount').textContent = '$' + parseFloat(this.dataset.totalAmount).toFixed(2);
          document.getElementById('read-paid-amount').textContent = '$' + parseFloat(this.dataset.paidAmount || 0).toFixed(2);
      ...
      document.querySelectorAll('[data-modal-target="paymentDebtModal"][data-modal-toggle]').forEach(button => {
        button.addEventListener('click', function() {
          document.getElementById('paymentDebtForm').action = paymentUrlTemplate.replace('__DEBT__', this.dataset.id);
        });
      });
